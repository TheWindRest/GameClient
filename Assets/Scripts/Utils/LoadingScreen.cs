...
public class LoadingScreen : MonoBehaviour
{
    ...
    public static string prefabName = "LoadPrefab";
    public static string SceneBundleName = "Scene";
    public static LuaFunction OnLoadFinished;
    ...
    // Load a new scene
    public static void LoadScene(string sceneName, LuaFunction luaFunction)
    {
        var prefab = ResourceManager.Instance.LoadPrefab("LoadingUI", prefabName);
        ...
        var result = ResourceManager.Instance.LoadScene(SceneBundleName);
        if(!result)
        {
            Debug.LogError("加载场景错误！！");
            return;
        }
        OnLoadFinished = luaFunction;
        instance.loadingProcess = SceneManager.LoadSceneAsync(sceneName);//, LoadSceneMode.Additive
        ...
        else
        {
            ...
            if (loadingProcess.isDone && enablePressAnyKey == false)
            {
                ...
                if (canvasAlpha.alpha <= 0)
                {
                    ...
                    {
                        OnLoadFinished.BeginPCall();
                        OnLoadFinished.PCall();
                        OnLoadFinished.EndPCall();
