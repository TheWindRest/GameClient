...
function _M:Connect(handler, listener)
    ...
    table.insert(self.handlers, {handler = handler, listener = listener})
    local t = { }
...
function _M:Fire(...)
    ...
    for k, v in pairs(self.handlers) do
        ...
        spawn(function()
            if v.listener then
                table.insert(self.args, 1, v.listener)
            end
            v.handler(unpack(self.args))
            i = i - 1
