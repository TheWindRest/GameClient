...
public class MapEditor : MonoBehaviour
{
    ...
    /// 第一层默认值
    /// </summary>
    public int LayerLevel1Default = 200;
    ...
    /// 历史地图状态
    /// </summary>
    private Dictionary<int, int[,]> historyMapStateDic = new Dictionary<int, int[,]>();
    ...
    {
        //加载列表
        obstaclerColor = SdataManage.Instance.LevelItemListData[0].GetDataDicColor();
        Level1ItemList = SdataManage.Instance.LevelItemListData[1].Data;
        Level2ItemList = SdataManage.Instance.LevelItemListData[2].Data;
        Level3ItemList = SdataManage.Instance.LevelItemListData[3].Data;
        ...
        if (Plane != null)
        {
            ...
            {
                MapHeight = mapLevel1.Length;
                MapWidth = mapLevel1[0].Length;
            }
            // 设置地图大小
            // 设置地缩放
            ...
            historyMapStateDic[LayerLevel1] = new int[MapHeight, MapWidth];
            historyMapStateDic[LayerLevel2] = new int[MapHeight, MapWidth];
            historyMapStateDic[LayerLevel3] = new int[MapHeight, MapWidth];
            // 初始化优化数据
    ...
    private void CreateObstacle()
    {
        ...
        if (Input.GetMouseButton(0))
        {
            ...
            if (Physics.Raycast(ray, out hit))
            {
                ...
                {
                    // 识别位置
                    var posOnMap = MapUtils.PositionToNum(Plane.transform.position, hit.point, UnitWidth, MapWidth, MapHeight);
    ...
    private void RefreshMap(int[][] map, int layer)
    {
        ...
        var historyMap = historyMapStateDic[layer];
        var rowCount = mapState.GetLength(0);
        var colCount = mapState.GetLength(1);
                ...
                if (AccessibilityId != val)
                {
                    ...
                    if (newObstacler == null)
                    {
                        ...
                        newObstacler = CreateObstacler(ObstaclerList == null ? null : ObstaclerList.transform,
                            ...
                            MapUtils.NumToPositionV(Plane.transform.position + Vector3.up * UnitWidth, // + new Vector3(UnitWidth * 0.5f, 0, UnitWidth * 0.5f),
                            new Vector2(col, row),
                            UnitWidth,
                            MapWidth,
                            MapHeight),
                    ...
                    // 变更不同层级单位的颜色
                    if (historyMap[row, col] != val)
                    {
                        historyMap[row, col] = val;
                        var meshRander = newObstacler.GetComponent<MeshRenderer>();
                        meshRander.material.color = obstaclerColor.ContainsKey(val) ? obstaclerColor[val] : GetRandomColor();
    ...
    {
        var random = new System.Random((int)(Time.realtimeSinceStartup * 10000));
        var result = new Color(random.Next(0, 255), random.Next(0, 255), random.Next(0, 255));
        return result / 255;
    }
